...
BEGIN
    ...
    -- Cleanup: Remove the test order with OID 5000 and 5001 if created
    BEGIN
        DELETE FROM "Order" WHERE OID IN (5000, 5001);
        DELETE FROM OrderProduct WHERE OID IN (5000, 5001);
        COMMIT;
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error during cleanup of Test Case 13: ' || SQLERRM);
    END;
