...
--                inserts necessary test data, and runs test cases for the create_order function.
-- ===========================================================================
...
-- ===========================================================================
BEGIN
    -- Inserir Tipos de Cliente
    BEGIN
        -- Verificar se os tipos de cliente já existem antes de inserir
        DECLARE
            v_count NUMBER;
        BEGIN
            ...
            IF v_count < 2 THEN
                ...
                DBMS_OUTPUT.PUT_LINE('Tipos de Cliente inseridos com sucesso.');
            ELSE
                DBMS_OUTPUT.PUT_LINE('Tipos de Cliente já existem. Pulando inserção.');
            END IF;
            COMMIT;
        END;
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            DBMS_OUTPUT.PUT_LINE('Erro ao inserir Tipos de Cliente: ' || SQLERRM);
    END;
    
    -- Inserir Clientes para Testes
    BEGIN
        -- Verificar se os clientes já existem antes de inserir
        DECLARE
        ...
        BEGIN
            ...
            IF v_count_active = 0 THEN
                -- Inserir Cliente Ativo
                INSERT INTO Client (NIF, Name, Email, Phone, IDClient, CID, Status) 
                VALUES ('PT501245488', 'Cliente Ativo', 'ativo@example.com', 912345678, 1, 'C1', 1);
                DBMS_OUTPUT.PUT_LINE('Cliente Ativo inserido com sucesso.');
            ELSE
                DBMS_OUTPUT.PUT_LINE('Cliente Ativo já existe. Pulando inserção.');
            END IF;
            
            SELECT COUNT(*) INTO v_count_inactive FROM Client WHERE NIF = 'PT501245987';
            IF v_count_inactive = 0 THEN
                -- Inserir Cliente Inicialmente Ativo (será inativado nos testes)
                INSERT INTO Client (NIF, Name, Email, Phone, IDClient, CID, Status) 
                VALUES ('PT501245987', 'Cliente Inativo', 'inativo@example.com', 987654321, 2, 'C2', 1); -- Status inicialmente ativo
                DBMS_OUTPUT.PUT_LINE('Cliente Inativo inserido com sucesso.');
            ELSE
                DBMS_OUTPUT.PUT_LINE('Cliente Inativo já existe. Pulando inserção.');
            END IF;
            COMMIT;
        END;
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            DBMS_OUTPUT.PUT_LINE('Erro ao inserir Clientes: ' || SQLERRM);
    END;
END;
/
...
-- ===========================================================================
-- Step 7: Create the create_order Function
-- ===========================================================================
...
BEGIN
    ...
    -- ---------------------------
    FOR i IN 1 .. p_product_list.COUNT LOOP
        INSERT INTO OrderProduct (OID, ProductPartID, Quantity) -- Corrigido ProductPartD para ProductPartID
...
BEGIN
    ...
    BEGIN
        UPDATE Client
        SET Status = 0
        WHERE NIF = 'PT501245987'; -- Certifique-se de que este NIF existe
        COMMIT;
    ...
    BEGIN
        UPDATE Client
        SET Status = 1
        WHERE NIF = 'PT501245987'; -- Revertendo para ativo
        COMMIT;
